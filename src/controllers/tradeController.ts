...
export const createTrade = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const tradeRecord: Trade[] = await Trade.findByBuyerSellerProductId(
      buyerId,
      product.ownerId,
      productId
    )
...
export const acceptTrade = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (trade.product.ownerId !== sellerId) {
      return res
        .status(403)
        .json({ message: 'Unauthorized to confirm this trade' })
    }
    ...
    if (!trade.canceledAt == null) {
      return res
        .status(400)
        .json({ message: 'Cannot confirm a canceled trade' })
    }
...
export const cancelTrade = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (
      trade.buyer.id !== req.user.userId &&
      trade.seller.id !== req.user.userId
    ) {
      return res
        .status(403)
        .json({ message: 'Unauthorized to cancel this trade' })
    }
...
export const endTrade = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!trade) {
      ...
      return res
        .status(400)
        .json({ message: 'Cannot end an unconfirmed trade' })
    } else if (currUserId !== trade.buyer.id) {
      return res.status(403).json({
        message:
          'Unauthorized to end this trade (Only Buyer can end the trade)',
      })
    }
...
export const getAllTradeRecord = async (
  ...
  try {
    ...
    res.status(200).json({
      buy: tradeRecordAsBuyer,
      sell: tradeRecordAsSeller,
    })
  } catch (error) {
    res.status(500).json({ message: error.message })
  }
